...
[Example]:
problem 1: ["Write a function that uses an `ArrayList`’s `forEach` and a `buffer`’s`write` / `alloc` methods to write all elements to the `buffer`."]

[Output Format]:
{{
    "problem": {{
      ["Problem statement with explicit cross-API usage"],
      ["Problem statement with explicit cross-API usage"],
      ["Problem statement with explicit cross-API usage"]
    }}
}}

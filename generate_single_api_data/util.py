...
def simulate_compile(file_path: str):
    try:
        subprocess.run(["feedback.bat", file_path], shell=True, check=True)

        errors = collect_error(r"research\log\feedback\hvigor_test_output.log",
                               r"research\log\feedback\hvigor_diagnostics.json")
        if errors:
            print(f"[SIMULATE] 检测到 {len(errors)} 个错误")
            return False, errors
        else:
            print("[SUCCESS] 编译通过，无错误")
            return True, None
    except subprocess.CalledProcessError as e:
        print(f"[SIMULATE ERROR] 编译执行失败: {e}")
